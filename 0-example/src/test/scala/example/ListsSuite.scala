...
class ListsSuite extends FunSuite {
  ...
  test("sum list with one item that value zero") {
    assert(sum(List(0)) === 0)
  }

  test("sum list with one item that value ten") {
    assert(sum(List(10)) === 10)
  }

  test("sum list with repeated items") {
    assert(sum(List(7, 7, 7, 8, 8)) === 37)
  }

  test("sum list with negative items") {
    assert(sum(List(7, -7, 10)) === 10)
  }
  ...
  test("max of one negative item") {
    assert(max(List(-72)) === -72)
  }

  test("max of when first item is the biggest") {
    assert(max(List(8, 4, 7, 2, 3)) === 8)
  }

  test("max of when last item is the biggest") {
    assert(max(List(4, 7, 2, 3, 9)) === 9)
  }

  test("max of repeated items") {
    assert(max(List(4, 7, 7, 3, 6)) === 7)
  }
